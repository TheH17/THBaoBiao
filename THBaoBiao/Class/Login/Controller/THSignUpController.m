//
//  THSignUpController.m
//  THBaoBiao
//
//  Created by 李浩鹏 on 16/9/25.
...
@implementation THSignUpController
...
-(void)keyboardDidShow:(NSNotification *)noti{
    ...
    CGPoint point = [_signUpButton convertPoint:CGPointZero toView:self.view];
...
-(void) layoutUI{
    ...
    labelView.image = [UIImage imageNamed:@"欢迎注册-文字"];
...
//    [checkButton sizeToFit];
    ...
    signUpButton.frame = CGRectMake((WIDTH - 270)/2, 505, 270, 50);
    signUpButton.backgroundColor = [UIColor colorWithRed:66/255.0 green:171/255.0 blue:255/255.0 alpha:1];
    ...
    [passButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(signUpButton.mas_bottom).with.offset(0.0225*HEIGHT);
        make.left.equalTo(self.view).with.offset(0.1333*WIDTH);
...
-(void)signUp{
    ...
    if ([_password.text isEqualToString:@""] || [_username.text isEqualToString:@""]) {
        ...
        [THLoginTool signUpWithParameters:params success:^(NSString *status, NSString *message) {
            [HUD hide:YES];
            if (![status isEqualToString:@"ok"]) {
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"注册失败" message:message delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
                [alertView show];
                return;
            }
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"注册成功" message:@"您已成功注册，请登录" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
            [alertView show];
            return;
        } failure:^(NSError *error) {
            [HUD hide:YES];
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"注册失败" message:@"网络故障请重试" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
            [alertView show];
            THLog(@"error:%@",error);
        }];
    }
